...
//! Return (used . size) of the NCG executable pool, in bytes.
...
Sexpr_t Platform_mop3_gc_diag(Lamb &lamb, Sexpr_t sexpr, Sexpr_t env_exec)
{
  ...
  //! Index 13/14: instantaneous cell-pool occupancy, for the idle/leak test.  The list
  ...
  //! positional.
  //! Index 15: B305 probe -- ncg_cons_slow_count, the number of times an NCG_CONS in generated
  //! native code fell to its SLOW path (ncg_cons_fast refused: Nfree <= yuasa_M, or sweeping).
  //! APPENDED at the tail, so it is consed FIRST -- the list is built by consing, so the first
  //! value pushed ends up LAST.  Never insert above this line: gc-pause-bench.scm and
  //! idle-heap-tests.scm read indices 0..14 POSITIONALLY.  A slow-path cons test that does not
  //! assert this counter MOVED has not shown it reached the path it names; see ll_vm_ncg.h.
  mop3_gc_protect(r, { r = lamb.cons(lamb.mk_integer((LL_int64) ncg_cons_slow_count, env_exec), r, env_exec); });
