...
//!   LLARDUINO  = 1  -> build against LAMBLISP'S OWN Arduino implementation ("LLArduino").
...
#define LL_ARDUINO       0
#endif
//! LLARDUINO was formerly called LL_FAKE_ARDUINO.  RENAMED because "fake" was wrong in the
...
//! enforces W^X, so this pool would be allocated, would pass any range check, would be USED, and
...
//! Do not widen the guard without running the probe's alias arm on a 4WD or WROVER first.
  ...
  #if LL_NCG_PSRAM_EXEC && LL_ESP32S3 && !LL_RISCV32
...
#if LLARDUINO
//! LLARDUINO Print / Stream / Serial.  NOT STUBS, and they need not be: Print and Stream are
...
//!
//! Do not "move them to PSRAM" by raising the size past CONFIG_SPIRAM_MALLOC_ALWAYSINTERNAL (4096).
//! That threshold governs capability-less malloc(); the UART driver does not use one.
